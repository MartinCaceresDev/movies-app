...
export const preparingLinks = (page, listObject, ids) => {
	let linksToFetch = [];
	if (page === 'Movies') {
		linksToFetch = ids[ids]?.map(
			(id) => `${link.movieBeforeMovieID}${id}${link.movieAfterMovieID}`
		);
	} else if (page === 'TV') {
		linksToFetch = ids[ids]?.map(
			(id) => `${link.tvShowBeforeID}${id}${link.tvShowAfterID}`
		);
		...
		if (listObject.name === 'Featured') {
			...
			let moviesLinks = ids.movies?.map(
				(id) => `${link.movieBeforeMovieID}${id}${link.movieAfterMovieID}`
			);
			let tvLinks = ids.tv?.map(
				(id) => `${link.tvShowBeforeID}${id}${link.tvShowAfterID}`
			);
